...
export default function enhanceRedux(models, { enhancer, reducer }) {
	...
	let states = {};
	let dispatch, getState, replaceReducer;

	/*
	*	包装 reducer 函数
	*/

	function decorateReducer(namespace, key, reducer) {
		return (...payload) => {
			if (!isProduction) {
	...
	*/
	...
	function middleware({ getState, dispatch }) {
		return (next) => (action) => {
			if (action[CALL_DISPATCH]) {
				...
				try {
					let [ namespace, key ] = type.split(NAMESPACE_SEP);
					const call = primevalReducer[namespace][key];
					const rootState = getState();
					return next({
						type: namespace,
						payload: call({ state: rootState[namespace], rootState }, ...payload)
					});
				} catch (err) {}
			}
			return next(action);
	...
	function defaultReduce(state = {}, action = {}) {
		...
		try {
			let typeList = type.split(NAMESPACE_SEP);
			const typeLastAttr = typeList.pop();
			const selectedItem = typeList.reduce((obj, attr) => {
				return obj[attr];
			}, state);
